...
import { processTransfer, type StockTransfer } from '@/features/inventory/transfers';
...
export function ProcessTransferDialog({
  ...
  async function handleConfirm() {
    setSubmitting(true);
    try {
      await processTransfer(transfer.id, { status: action, note: note.trim() || undefined });
      toastSuccess(`Transfer ${meta.confirm.toLowerCase()}`);
      onProcessed();
    } catch (err) {
      errorAlert(apiErrorMessage(err, 'Gagal memproses transfer'));
    } finally {
      setSubmitting(false);
    }
  }
